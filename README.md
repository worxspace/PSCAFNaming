...
This module implements a set of methodologies for naming convention implementation specifically the default Microsoft Cloud Adoption Framework for Azure recommendations as per https://docs.microsoft.com/en-us/azure/cloud-adoption-framework/ready/azure-best-practices/naming-and-tagging.

## Installing the Module

You can simply install the module from the Powershell Gallery as follow:

```powershell
Install-Module PSCAFNaming
```

The `New-CAFResourceName` cmdlet allows you to:
...
* **random_length** (coming soon) - default to ``0`` : configure additional characters to append to the generated resource name. Random characters will remain compliant with the set of allowed characters per resources and will be appended before suffix(ess).
* **random_seed** (coming soon) - default to ``0`` : Define the seed to be used for random generator. 0 will not be respected and will generate a seed based in the unix time of the generation.
* **ResourceType** (required) -  describes the type of azure resource you are requesting a name from (eg. azure container registry: azurerm_container_registry). See the Resource Type section
...
* **CleanInput** (optional) - defaults to ``true``. remove any noncompliant character from the name, suffix or prefix.
* **IgnoreValidations** (optional) - defaults to ``false``. Enables the passthrough mode - in that case only the clean input option is considered.
* **AddResourceAbbreviation** (optional) - defaults to ``true``. If a slug should be added to the name - If you put false no slug (the few letters that identify the resource type) will be added to the name.
...
The comprehensive list of resource type can be found [here](https://github.com/aztfmod/terraform-provider-azurecaf/docs/resources/azurecaf_name.md)

## Related repositories

| Repo                                                                                             | Description                                                                   |
|--------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------|
| [azure_caf_provider](https://github.com/aztfmod/terraform-provider-azurecaf)                     | custom terraform provider for naming conventions. Inspiration for this module |
